...
case class Q5(sqlContext: SQLContext) extends TPCHQuery(sqlContext) {
  ...
  lazy val n_r = region.filter('r_name === "ASIA").join(nation, 'r_regionkey === 'n_regionkey)
  lazy val s_n_r = supplier.join(n_r, 'n_nationkey === 's_nationkey)
  lazy val l_s_n_r = lineitem.join(s_n_r, 's_suppkey === 'l_suppkey)
  lazy val o_c = ord.join(customer, 'o_custkey === 'c_custkey)

  def result =
    l_s_n_r.join(o_c, 'l_orderkey === 'o_orderkey && 's_nationkey === 'c_nationkey)
...
case class Q8(sqlContext: SQLContext) extends TPCHQuery(sqlContext) {
  ...
  lazy val l_s_n = lineitem.join(supp_na, 'l_suppkey === 's_suppkey)
  lazy val l_s_n_p = l_s_n.join(pa, 'l_partkey === 'p_partkey)

  lazy val cust_na_reg = customer.join(nation, 'c_nationkey === 'n_nationkey)
    .join(reg, 'n_regionkey === 'r_regionkey)
  lazy val o_c_n_r = ord.join(cust_na_reg, 'o_custkey === 'c_custkey)

  lazy val all_nations =
    l_s_n_p.join(o_c_n_r, 'l_orderkey === 'o_orderkey)
      .select(
        'o_orderdate.substr(0, 4).as("o_year"),
        ('l_extendedprice * (lit(1) - 'l_discount)).as("volume"),
        l_s_n_p("n_name").as("nation"))
...
case class Q9(sqlContext: SQLContext) extends TPCHQuery(sqlContext) {
  ...
  lazy val s_n = supplier.join(nation, 's_nationkey === 'n_nationkey)
  lazy val l_s_n = lineitem.join(s_n, 's_suppkey === 'l_suppkey)
  lazy val pa = part.filter('p_name.contains("green"))
  lazy val l_s_n_p = l_s_n.join(pa, 'p_partkey === 'l_partkey)
  lazy val l_s_n_p_ps = l_s_n_p.join(partsupp, 'l_suppkey === 'ps_suppkey && 'l_partkey === 'ps_partkey)

  def result =
    l_s_n_p_ps.join(orders, 'o_orderkey === 'l_orderkey)
      .select(
        'n_name,
        'o_orderdate.substr(0, 4).as("o_year"),
